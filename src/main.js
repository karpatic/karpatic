...
let w = window;
w.sendPing = async (event=false) => { w.content && navigator.sendBeacon('https://ping.charleskarpati.com/', `{"from":"${window.oldRoute||w.location.href}","to":"${event?.href||event?.target?.href||location.pathname}"}` ) }

w.updateRedirectListeners = () => { document.querySelectorAll('a[href^="./"]').forEach(l=>[l.removeEventListener,l.addEventListener].forEach(f=>f.call(l,'click',redirect))) }
