...
export const navEvent = async () => {
  ...
  // Reload
  if (href.split("#")[0] != w.href?.split("#")[0]) await handleRoute(), (w.href = href);
};
...
export const handleRoute = async () => {
  ...
  w.toast || (await import(/* webpackChunkName: "template" */ "./template.js"));
  ...
  let content = await (!isLocal || preRendering
    ...
    : (await import(/* webpackChunkName: "convert" */ "./convert.mjs")).ipynb_publish(`/ipynb/${route}.ipynb`));
  ...
  w.meta.template ||= "template_article";
  if (meta.template !== document.body.getAttribute("data-template")) {
    document.body.setAttribute("data-template", meta.template);
    document.body.innerHTML = await (await fetch(`/templates/${meta.template}.html`)).text();

    // Add Basic Stylesheet
    document.body.insertAdjacentHTML(
      "beforeend",
      `<style>${await (await fetch(`${w.location.origin}/templates/${meta.template}.css`)).text()}</style>`
    );
  ...
  if (w.sitemap && !w.meta.hide_sitemap) {
    if (!w.sitemap_content) {
      let txt = await (await fetch(`${location.origin}/templates/${w.meta.template}_sitemap.css`)).text();
      document.body.insertAdjacentHTML("beforeend", `<style>${txt}</style>`);
    }
    if (w.sm_name != sm) {
      w.sm_name = sm;
      w.sitemap_content = await (await fetch(`/posts/${sm_name}_map.json`)).json();
    }
  }
