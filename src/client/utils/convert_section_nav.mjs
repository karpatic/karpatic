...
async function generate_sectionmap(pages, directory) {
  ...
  const sitemapPath = `${SAVETO}${directory}_map.json`; 
...
async function ipynb_publish(
  ...
  if (type === "json") {
    ...
    // console.log('START:', {saveDir, file, type}, Object.keys(final.meta))
    let { filename, ...meta } = final.meta;
    //console.log(final)
    filename = filename || filePath.split("/")[filePath.split("/").length - 1]
    name = filename = (filename).toLowerCase().replaceAll(" ", "_");
    final.meta = { ...meta, filename };
  }
  ///console.log(('Saving ', final, '\n\n');
  ///console.log(('As: ', file, '\n')
  ///console.log(('To ', saveDir, '\n\n');
  const t = path.join(saveDir, `${name}.${type}`);
