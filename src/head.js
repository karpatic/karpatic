...
(async () => {
  ...
  const resolveRouteMeta = async ({ hr, page, url, filename }) => {
    ...
    const cms = Array.isArray(notes)
      ...
      : Object.values(notes || {}).find(x => (x?.filename || "").toLowerCase() === filename.toLowerCase());
  ...
  const page =
    location.pathname.replace("/docs/", "/").replace("/", "").replace(/\/$/, "").replace(".html", "") || "index"; // removes leading and trailing slashes
  ...
  let url = `/${(hr.pwapages.split(",").some(x => page == x) ? "" : "rsc/posts/") + page}.json`;
  ...
  ReactDOMServer.renderToString(
    <HelmetProvider>
      <Helmet>
        <meta charset="UTF-8" />
        <meta http-equiv="Content-Security-Policy" content="img-src * data:; connect-src *;" />
