...
(async () => {
  ...
  const page = location.pathname.replace("/", "").replace(/\/$/, "").replace(".html", "") || "index";
  ...
  const url = `/${(hr.pwapages.split(",").some((x) => page == x) ? "" : "posts/") + page}.json`;

  // Merge to defaults
  try {
    hr = { ...hr, ...(await (await fetch(url)).json()).meta };
  } catch (e) {
    console.error("<~~~~~ HEADER CONFIG DATA ERROR ~~~~~>", e, location.pathname, url);
  }
