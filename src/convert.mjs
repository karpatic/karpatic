...
function cleanCell(cell, meta) {
  ...
  if (cell["cell_type"] == "markdown") {
    ...
    x = x.replace(/<li>(.*?)<\/li>/g, "<li><div>$1</div></li>"); // wrap li's in p's
    x = x.replace(/<pre><code>([\s\S]*?)<\/code><\/pre>/g, "<pre class='prettyprint'>$1</pre>"); // replace code with pre.prettyprint

    console.log(x);
    x = x.replace(/<a\s+(?:[^>]*?\s+)?href="(.*?)"/g, (match, href) => {
