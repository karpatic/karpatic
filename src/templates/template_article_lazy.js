...
createCube = () => {
  const x = rir(0, 200) + "svh";
  const y = rir(0, 100) + "svh";
...
createCubes = () => {
  return Array(rir(8, 15))
    .fill()
    .map(() => {
      c = window.createCube();
      window.svg_bg.appendChild(c);
      return c;
    });
};
animateCubes = () => {
  [...document.querySelectorAll(".cube")].forEach((cube) => {
    c = cube.style;
    c.setProperty("--startx", `-${rir(0, 10)}svh`);
    c.setProperty("--endx", `${rir(0, 10)}svh`);
    c.setProperty("--starty", `-${rir(100, 125)}svh`);
    c.setProperty("--endy", `${rir(100, 125)}svh`);
    cube.style.animation = `cube-animation ${duration}ms forwards`;
  });
};
window.duration = 20000;
createAndAnimate = () => {
  window.svg_bg.innerHTML = "";
  window.cubes = createCubes(rir(15, 25));
  animateCubes();
};
createAndAnimate();
setInterval(() => {
  createAndAnimate(duration);
}, duration);
