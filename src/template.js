...
const populateTemplate = async () => {
  ...
  meta.breadcrumbs = [
    ...
    location.pathname
      ...
      .map((x, i) => {
        x = x.replace(".html", "");
        return x == "index" ? "" : `<a href=${create_url(x, w.sm_name)}.html>${capitalize(x.replace(".html", ""))}</a>`;
      })
      .join("/"),
  ].join("/");
  ...
  [...document.querySelectorAll("h2, h3, h4")].forEach((header) => {
    ...
    anchor.setAttribute(
      "onclick",
      `navigator.clipboard.writeText('https://charleskarpati.com${location.pathname + location.hash}'); w.toast?.();`
    );
  ...
  // Restart Observer on new page if included in template_article_lazy
  loadObserver?.();
...
let create_url = (link, sitemap) => {
  ...
  let t = `./${(fromSubpath && !toSubpath && "../") || (!fromSubpath && toSubpath && sitemap + "/") || ""}${link}`;
